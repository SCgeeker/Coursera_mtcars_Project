...
Firstly I explored the correlations for each pair of variables in `mtcars` dataset. As the below plot presented, `cyl`, `vs`, `am`, `gear`, and `carb` are categorical variables.  
```{r plot, echo=FALSE, results='asis', fig.align='center'}
pairs(mtcars, panel = panel.smooth, col = 3 + mtcars$am)
```

The original `mtcars` dataset was duplicated to new dataset `mtcars1` and the five categorical variables were transformed into `factor`. The model included all variables was stored in `all`:
