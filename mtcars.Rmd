...
```{r loadRequired, cache=TRUE, echo=FALSE, results='hide', message=FALSE, warning=FALSE}
library(MASS)
```
...
```{r fit, cache=TRUE, echo=TRUE, results='hide', warning=FALSE, message=FALSE}
fit <- stepAIC(all, direction="both")
```
```{r fitsum, cache=TRUE, echo=FALSE, results='markup', warning=FALSE, message=FALSE}
summary(lm(mpg ~ wt + qsec + am, mtcars1))$coef
```
### Diagnostics of Residuals
Below plots illustrated that the residuals of most observations obey the normalization assumption in addition to **`r rownames(mtcars)[c(17, 18, 20)]`**. The plot compared resesuals and leverages indicates that the three observations contributed the least influence. Therefore, I indicated **`r rownames(mtcars)[c(17, 18, 20)]`** as the outliers of `mtcars` dataset.  
